...
<script>
	message_taille_fichier = "Le fichier ne doit pas dépasser <%=CDbl(Session("iMaxSizeUpload"))%> Mo !"
</script>

<%
IF CDbl(Request.ServerVariables("CONTENT_LENGTH")) > (CDbl(Session("iMaxSizeUpload"))*1000*1000) THEN
	Response.Write("<script> Popup_Alert(message_taille_fichier, 'window.history.back()');</script>")
	Response.End
END IF
...
' *****************************************************************************
	Var_chemin_documents_temp = Session("sTmpDir")
	
	Var_chemin_site	= Session("Chemin_Application")
...
Class UplFichier
	...
	Public Property Get ChampForm(Lequel)
		For i = 1 To UBound(NomChampTXT)
			If NomChampTXT(i) = Lequel Then
				ChampForm = RepareUTF8(LesChampTXT(i))
				Exit For
			End If
		Next
	End Property
	...
	Private Sub Class_Initialize()
		...
		Redim LesChampTXT(0)
		Redim NomChampTXT(0)
		CheminLocal = Var_chemin_documents_temp ' Server.MapPath(".\") & "\" ' Dossier d'upload par defaut
