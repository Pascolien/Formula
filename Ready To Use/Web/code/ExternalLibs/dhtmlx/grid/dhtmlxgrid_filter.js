...
dhtmlXGridObject.prototype._filterA = function (column, value) {
    ...
    for (var i = this.rowsBuffer.length - 1; i >= 0; i--) {
        sCellValue = Replace_From_XML(this._get_cell_value(this.rowsBuffer[i], column).toString());
        if (d ? (sCellValue.toLowerCase().indexOf(value) == -1) : (!value.call(this, this._get_cell_value(this.rowsBuffer[i], column), this.rowsBuffer[i].idd)))
...
function DeleteDoublons(AArray) {
    try {
        return AArray.sort().filter(function (item, pos) {
            return !pos || item != AArray[pos - 1];
        })
    } catch (e) {
        var i, j, len = AArray.length, out = [], obj = {};
        for (i = 0; i < len; i++) {
            obj[AArray[i]] = 0;
        }
        for (j in obj) {
