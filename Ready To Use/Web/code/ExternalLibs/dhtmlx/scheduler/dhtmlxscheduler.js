...
dhtmlxAjax={get:function(A,C)
    {var B=new dtmlXMLLoaderObject(true);
        ...
        B.waitCall=C;B.loadXML(A);
        return B;},post:function(A,C,D)
    {var B=new dtmlXMLLoaderObject(true);
        B.async=(arguments.length<4);
        B.waitCall=D;
        B.loadXML(A,true,C);
        return B;},getSync:function(A)
    {return this.get(A,null,true);},postSync:function(A,B)
    {return this.post(A,B,null,true);}};
function dtmlXMLLoaderObject(B,D,C,A)
{this.xmlDoc="";if(typeof (C)!="undefined")
    {this.async=C;}else{this.async=true;}this.onloadAction=B||null;
    this.mainObject=D||null;
    this.waitCall=null;
    this.rSeed=A||false;return this;}
...
{var A=true;this.check=function()
    {if((B)&&(B.onloadAction!=null))
        {if((!B.xmlDoc.readyState)||(B.xmlDoc.readyState==4))
            {if(!A){return ;}A=false;if(typeof B.onloadAction=="function")
                {B.onloadAction(B.mainObject,null,null,null,B);}
                if(B.waitCall){B.waitCall.call(this,B);
                    B.waitCall=null;}}}};
    return this.check;};
...
{if(this.xmlDoc.responseXML)
    {var B=this.xmlDoc.responseXML.getElementsByTagName(C);
        ...
        {var B=this.xmlDoc.responseXML.getElementsByTagName((C.split(":"))
            [1]);}var E=B[0];}
    else{var E=this.xmlDoc.documentElement;}
    if(E){this._retry=false;return E;}if((_isIE)&&(!this._retry))
    {var D=this.xmlDoc.responseText;
        var A=this.xmlDoc;this._retry=true;
        this.xmlDoc=new ActiveXObject("Microsoft.XMLDOM");
        this.xmlDoc.async=false;
        this.xmlDoc.loadXML(D);
        return this.getXMLTopNode(C,A);}dhtmlxError.throwError("Scheduler: unable to load the data","Incorrect XML",[(A||this.xmlDoc),this.mainObject]);
    return document.createElement("DIV");};
dtmlXMLLoaderObject.prototype.loadXMLString=function(B)
{try{var C=new DOMParser();
        this.xmlDoc=C.parseFromString(B,"text/xml");}
    catch(A){this.xmlDoc=new ActiveXObject("Microsoft.XMLDOM");
        this.xmlDoc.async=this.async;
        this.xmlDoc.loadXML(B);}this.onloadAction(this.mainObject,null,null,null,this);
    if(this.waitCall){this.waitCall();
        this.waitCall=null;}};
dtmlXMLLoaderObject.prototype.loadXML=function(C,B,A,D)
{if(this.rSeed){C+=((C.indexOf("?")!=-1)?"&":"?")+"a_dhx_rSeed="+(new Date()).valueOf();}this.filePath=C;if((!_isIE)&&(window.XMLHttpRequest)){this.xmlDoc=new XMLHttpRequest();}else{if(document.implementation&&document.implementation.createDocument){this.xmlDoc=document.implementation.createDocument("","",null);
            this.xmlDoc.onload=new this.waitLoadFunction(this);
            this.xmlDoc.load(C);
            return;}else{this.xmlDoc=new ActiveXObject("Microsoft.XMLHTTP");}}
    if(this.async){this.xmlDoc.onreadystatechange=new this.waitLoadFunction(this);}
    this.xmlDoc.open(B?"POST":"GET",C,this.async);
    if(D){this.xmlDoc.setRequestHeader("User-Agent","dhtmlxRPC v0.1 ("+navigator.userAgent+")");
        this.xmlDoc.setRequestHeader("Content-type","text/xml");}
    else{if(B){this.xmlDoc.setRequestHeader("Content-type","application/x-www-form-urlencoded");}}
...
{this.onloadAction=null;
    ...
    this.xmlDoc=null;
    return null;};
dtmlXMLLoaderObject.prototype.xmlNodeToJSON=function(D){var C={};
    for(var B=0;
  B<D.attributes.length;B++)
  {
  C[D.attributes[B].name]=D.attributes[B].value;}
  C._tagvalue=D.firstChild?D.firstChild.nodeValue:"";
  for(var B=0;B<D.childNodes.length;B++)
  {var A=D.childNodes[B].tagName;if(A){if(!C[A]){C[A]=[];}C[A].push(this.xmlNodeToJSON(D.childNodes[B]));}}
  return C;};
function callerFunction(A,B)
{this.handler=function(C)
    {if(!C){C=window.event;}A(C,B);
        return true;};
    return this.handler;}
function getAbsoluteLeft(B)
{
    ...
    while(A!=null)
    {C+=A.offsetLeft;A=A.offsetParent;}return C;}
function getAbsoluteTop(C)
{return getOffset(C).top;
    var B=C.offsetTop;
    var A=C.offsetParent;
    while(A!=null){B+=A.offsetTop;A=A.offsetParent;}return B;}
...
{var C=0,B=0;
    while(A)
    {
        ...
        A=A.offsetParent;}return{top:C,left:B};
}
...
function convertStringToBoolean(A)
{
    ...
    switch(A){case"1":case"true":case"yes":case"y":case 1:case true:return true;break;default:return false;}}
         function  getUrlSymbol(A){if(A.indexOf("?")!=-1){return"&"}else{return"?"}}
         function dhtmlDragAndDropObject(){
        	 if(window.dhtmlDragAndDrop){return window.dhtmlDragAndDrop}
        	 this.lastLanding=0;this.dragNode=0;
        	 this.dragStartNode=0;this.dragStartObject=0;
        	 this.tempDOMU=null;this.tempDOMM=null;this.waitDrag=0;
        	 window.dhtmlDragAndDrop=this;return this}
         ...
         if(A.indexOf("_icon")!=-1){scheduler._click.buttons[A.split(" ")[1].replace("icon_","")](D)}}
                 ...
                 dhx_cal_tab:function(){var A=this.getAttribute("name").split("_")[0];
         ...
         scheduler.render_event=function(I){
        	 ...
        	 var C=I._sorder*B+1;
        	 if(!I._inner){B=B*(I._count-I._sorder)}
        	 ...
        	 H+=this._dy_shift;if(this._edit_id==I.id){B=Math.max(B-4,140);
        	 ...
        	 A.style.cssText+=";margin:2px 2px 2px 2px;overflow:hidden;";G.appendChild(A);this._els.dhx_cal_data[0].appendChild(G);this._rendered.push(G);A.innerHTML="<textarea class='dhx_cal_editor'>"+I.text+"</textarea>";if(this._quirks7){A.firstChild.style.height=L-12+"px"}this._editor=A.firstChild;this._editor.onkeypress=function(N){if((N||event).shiftKey){return true}var M=(N||event).keyCode;if(M==scheduler.keys.edit_save){scheduler.editStop(true)}if(M==scheduler.keys.edit_cancel){scheduler.editStop(false)}};this._editor.onselectstart=function(M){return(M||event).cancelBubble=true};A.firstChild.focus();this._els.dhx_cal_data[0].scrollLeft=0;A.firstChild.select()}
         ...
         scheduler._init_lightbox_events=function(){
        	 this._get_lightbox().onclick=function(C){
        		 ...
        		 if(E&&E.className){
        			 switch(E.className){
        			 case"dhx_save_btn":if(scheduler.checkEvent("onEventSave")&&!scheduler.callEvent("onEventSave",[scheduler._lightbox_id,scheduler._lightbox_out({})])){return}alert("Successfully Updated");scheduler._empty_lightbox();scheduler.hide_lightbox();scheduler._lightbox = null;break;
        			 case"dhx_delete_btn":var F=scheduler.locale.labels.confirm_deleting;if(!F||confirm(F)){var id=scheduler.getId;scheduler.deleteEvent(scheduler._lightbox_id);scheduler.hide_lightbox()}scheduler._lightbox = null;break;
        			 case"dhx_cancel_btn":scheduler.callEvent("onEventCancel",[scheduler._lightbox_id]);scheduler._edit_stop_event(scheduler.getEvent(scheduler._lightbox_id),false);scheduler.hide_lightbox();scheduler._lightbox = null;break;default:if(E.className.indexOf("dhx_custom_button_")!=-1){var A=E.parentNode.getAttribute("index");var D=scheduler.form_blocks[scheduler.config.lightbox.sections[A].type];var B=E.parentNode.parentNode;D.button_click(A,E,B,B.nextSibling)}}}};this._get_lightbox().onkeypress=function(A){switch((A||event).keyCode){case scheduler.keys.edit_save:if((A||event).shiftKey){return}scheduler._empty_lightbox();scheduler.hide_lightbox();break;case scheduler.keys.edit_cancel:scheduler._edit_stop_event(scheduler.getEvent(scheduler._lightbox_id),false);scheduler.hide_lightbox();break}}};scheduler.setLightboxSize=function(){var B=this._lightbox;if(!B){return}var A=B.childNodes[1];A.style.height="0px";A.style.height=A.scrollHeight+"px";B.style.height=A.scrollHeight+50+"px";A.style.height=A.scrollHeight+"px"};

         scheduler._get_lightbox=function(){
         var eventId=""+scheduler._select_id;
         if (eventId!=null&&eventId.indexOf("_")>0) eventId=eventId.substring(0, eventId.indexOf("_"));
         if(!this._lightbox){
             var G=document.createElement("DIV");G.className="dhx_cal_light";
             if(/msie|MSIE 6/.test(navigator.userAgent)){G.className+=" dhx_ie6"}
             ...
             for(var B=0;B<E.length;B++){
                 ...
                 var A="";
                 if(E[B].button){A="<div style='float:right;' class='dhx_custom_button' index='"+B+
                         "'><div class='dhx_custom_button_"+E[B].name+"'></div><div>"+
                         this.locale.labels["button_"+E[B].button]+"</div></div>"}
                 C+="<div id='"+E[B].id+"' class='dhx_cal_lsection'>"+A+this.locale.labels["section_"+E[B].name]+
                     "</div>"+F.render.call(this,E[B]);
             }
             ...
             D[1].innerHTML=C;*/
             document.getElementById("div_lb_descr").innerHTML=C;
             document.getElementById("div_lb_sav_btn").innerHTML=scheduler.locale.labels.icon_save;
             document.getElementById("div_lb_cnc_btn").innerHTML=scheduler.locale.labels.icon_cancel;
             document.getElementById("div_lb_del_btn").innerHTML=scheduler.locale.labels.icon_delete;
//             document.getElementById("div_lb_tree").innerHTML="<iframe id='iframe_lb_tree' class=" +
//                 "'dhx_cal_iframe_tree' src='treeInPopUp.jsp?id="+eventId+"'></iframe>"

             try {
                 //treeCal = initTree("div_lb_tree");
                 //treeCal.setXMLAutoLoading("loadTreeForEvent.do");
                 //treeCal.loadXML("loadTreeForEvent.do?id=0");
                 //initTreeForCalendar(treeCal);
             }
             catch(exc) {
              alert("Unable to load the Object Tree:\n" + exc);
            }

             // BAS_MODIF_START: Modifié par Julien (permet de prendre en compte l'ajout de "Link to")
        ...
        scheduler._lightbox_template=
            ...
            "</div>"+
            "<div id='div_lb_descr' class='dhx_cal_larea'></div>" +
            "<div class='dhx_div_linkto'>"+
              "<div class='dhx_linkto_bar'>"+
                "<span style='display: inline; float: left;'>Link to</span>"+
//                "<a class='dhx_a_switch_btn' href='javascript:switchTreeList();'>"+
                "<a class='dhx_a_switch_btn' href='.' onclick='switchView(); return false;'>"+
                "<img id='img_switch' alt='switch' src='../../resources/theme/img/btn_tree/16x16-Tree_View.png' class='dhx_switch_btn'></a>"+
              "</div>"+
              "<div id='scheduler_popup_tree'>"+
                "<div id='div_lb_tree' class='dhx_cal_iframe_tree' style='height: 175px; overflow: hidden;'></div>"+
                "<div id='div_lb_list' class='dhx_cal_iframe_tree' style='padding-top:2px;'></div>"+
              "</div>"+
            "</div>"+
            "<div class='dhx_btn_set'>"+
              "<div class='dhx_save_btn'></div>" +
              "<div id='div_lb_sav_btn'>&nbsp;</div>"+
            "</div>"+
            "<div class='dhx_btn_set'>"+
              "<div class='dhx_cancel_btn'></div>"+
              "<div id='div_lb_cnc_btn'>&nbsp;</div>"+
            "</div>"+
            "<div class='dhx_btn_set' style='float:right;'>" +
              "<div class='dhx_delete_btn'></div>"+
              "<div id='div_lb_del_btn'>&nbsp;</div>"+
            "</div>";
          /*
            "<div class='dhx_div_duration'>"+
              "Duration:&nbsp;"+
              "<input id='inp_duration' class='dhx_input_duration' type='text' "+
              "onKeyPress='var c=Math.max(event.charCode||0,event.keyCode||0);"+
              "if(c==27) setDuration(\"redisplay\"); if(c==13) setDuration(\"text\");' "+
              "onChange='setDuration(\"redisplay\");'/>"+
            "</div>";
          */
        scheduler._dp_init=function(A){A._methods=["setEventTextStyle","","changeEventId","deleteEvent"];this.attachEvent("onEventAdded",function(B){if(!this._loading&&this.validId(B)){A.setUpdated(B,true,"inserted")}});this.attachEvent("onBeforeEventDelete",function(C){if(!this.validId(C)){return}var B=A.getState(C);if(B=="inserted"){A.setUpdated(C,false);return true}if(B=="deleted"){return false}if(B=="true_deleted"){return true}A.setUpdated(C,true,"deleted");return false});this.attachEvent("onEventChanged",function(B){if(!this._loading&&this.validId(B)){A.setUpdated(B,true,"updated")}});A._getRowData=function(F,B){B=B||"";var D=this.obj.getEvent(F);var E=[];for(var C in D){if(C.indexOf("_")==0){continue}if(D[C]&&D[C].getUTCFullYear){E.push(C+"="+this.obj.templates.xml_format(D[C]))}else{E.push(C+"="+this.escape(D[C]))}}return B+E.join("&"+B)};A._clearUpdateFlag=function(){}};scheduler.setUserData=function(C,A,B){this.getEvent(C)[A]=B};scheduler.getUserData=function(B,A){return this.getEvent(B)[A]};scheduler.setEventTextStyle=function(C,A){this.for_rendered(C,function(D){D.style.cssText+=";"+A});var B=this.getEvent(C);B._text_style=A;this.event_updated(B)};scheduler.validId=function(A){return true};
