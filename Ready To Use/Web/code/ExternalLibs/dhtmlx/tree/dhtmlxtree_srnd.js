...
"text", a) && this.reParse(c); if (d.id == c.id) break; if (b && e.length == 1 && e[0][1] == 0) break
} return null
};
this.deleteChildItems = function (a) {
    if (this.rootId == a && (this._selected = [], this._idpull = {}, this._p = this._pos_c = this._pullSize = null, this.allTree.appendChild(this.selectionBar), this.allTree.removeChild(this.htmlNode.htmlNode), this.htmlNode = new dhtmlXTreeItemObject(this.rootId, "", 0, this), this.htmlNode.htmlNode.childNodes[0].childNodes[0].style.display = "none", this.htmlNode.htmlNode.childNodes[0].childNodes[0].childNodes[0].className = "hiddenRow", this.allTree.insertBefore(this.htmlNode.htmlNode,
this.selectionBar), _isFF)) this.allTree.childNodes[0].width = "100%", this.allTree.childNodes[0].style.overflow = "hidden"
};
    this._HideShow = function (a, b) {
        if (this.XMLsource && !a.XMLload) { if (b != 1) a.XMLload = 1, this._loadDynXML(a.id) } else if (a.span || this._buildSRND(a), a.unParsed && (this.reParse(a), this.prepareSR(a.id)), a.childsCount != 0) {
            var c = a.htmlNode.childNodes[0].childNodes, d = c.length; if (d > 1) {
                if ((c[1].style.display != "none" || b == 1) && b != 2) {
