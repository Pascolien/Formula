...
begin
  ...
  try
    try
      ...
      sTag_Formula := ObjJSONFormula.GetValue('sTag_Formula').Value;

      if TxFormulaStoreMode(StrToInt(ObjJSONFormula.GetValue('iStore_Mode').Value)) =  fsmObjectSettings then
      try
        _ObjectSettings.Write(id_Formula, sTag_Formula, ObjJSONFormula.ToString);
...
begin
  ...
  JSONInputObject := TJSONObject.ParseJSONValue(VarRecToStr(AArr_Input[1])) as TJSONObject;

  id_Obj := StrToInt(JSONInputObject.GetValue('ID_Object').Value);
  ID_AS_Formula := StrToInt(JSONInputObject.GetValue('ID_AS').Value);
  ID_Att_Inifile := StrToInt(JSONInputObject.GetValue('ID_Att_Inifile').Value);
  iStoreMode := TxFormulaStoreMode(StrToInt(JSONInputObject.GetValue('iStore_Mode').Value));
  sTagFormula := JSONInputObject.GetValue('sTag_Formula').Value;

  DoInitialize_Dll(VarrecToStr(AArr_Input[0]));

  if iStoreMode = fsmObjectSettings then
  begin
    sJSON_Formula := _ObjectSettings.Read(Id_Obj, sTagFormula);
