...
>>>>>>> .r31842
    ...
    this.checkDisplay = function () {
        ...
        divDisplay.setAttribute("onmouseout", ObjFormula.calculate);
        //divDisplay.setAttribute("onmouseover",ObjFormula.calculate);
            ...
            success: function (data) {
                ...
                if (sResult[0] === 'ok') {
                    ...
                    if (json != undefined) {
                        ...
                        self.ID_Formula = aInput.ID_Object;
                        self.iDisplay_Format = aInput.iDisplay_Format;
                    }
                    self.loaded = true;
                }
            },
        ...
        if (this.iDisplay_Format == 0) {
            ...
            this.variables.forEach(function (variable) {
                ...
                input.addEventListener('change', function () {
                    ...
                    if (sValue == "") {
                        dhtmlx.alert({text:"Please enter a value"});
                        return false
                    }
                    if (isNaN(sValue)) {
                        dhtmlx.alert({text:"Please enter a numerical value"});
                        return false
                    } else {
                ...
                if (stage == 2) {
                    if (Number(nValue.replace(',', '.')) >= 0) {
                        ...
                        if ((FormulasGrid.cells(rId, 2).cell.textContent === "%") && (nValue.replace(',', '.') > 100)) {
                            dhtmlx.alert({text:"Pourcentage trop grand."});
                            return false;
            ...
            FormulasGrid.attachEvent("onRowSelect", function (id, ind) {
                var rowIndex = FormulasGrid.getRowIndex(id);
                selectedVariable = self.getVariable(rowIndex);
            });
